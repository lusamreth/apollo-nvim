...
function add_border(content,opts)

    local columns = vim.api.nvim_get_option("columns")
    local opts = opts or {}
    
    --early return2
    if opts.border == false then return end
    ...
    local max_length = find_max(len_arr)
    ...
    for i,line in pairs(content) do
        lines[i] = string.format('%s%s%s%s%s%s',
        BORDERVERTICAL,
        ----------------------------------------
        pleft,
        line,                                     -- content line
...
function apply_padding(line,padding)
