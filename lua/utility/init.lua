...
function Max(a) return vim.fn.max(a) end
function Min(a) return vim.fn.min(a) end
...
local function make_diagnostic_stat(pos, config, start_idx)
    local i = 1
    if start_idx ~= nil then local i = start_idx end
        ...
        require'galaxyline'.section[pos][i] =
            {
                [name] = {
                    ...
                    provider = function()
                        ...
                        p = string.format("%s [%s] ", all_trim(val.sign),
                                          all_trim(p))
                        return p
                    end,
                    -- icon = val.sign,
...
local function default()
    local defaultcfg = make_stat_conf({
        " :" .. diag_col.red, " :" .. diag_col.yellow,
        " :" .. diag_col.redwine, " :" .. diag_col.teal
    })
